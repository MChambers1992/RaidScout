...
### Added
- **`role: 'auto'` resolves in one request for most characters** — the dps response already carries the ranked spec, so the hps query is issued only for characters that need it (a healer spec, or a response that reported nothing at all and so can't be told apart from a healer with no damage rankings). Asking for both metrics up front roughly doubled the WarcraftLogs points cost of scoring a list, which matters most on WoWProgress where every row uses `auto`. Pinned by `tests/wcl-api.test.js`, which now covers the response parsers `auto` depends on.
