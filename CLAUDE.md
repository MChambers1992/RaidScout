...
1b. **WarcraftLogs API Client** (`src/wcl-api.js`) — imported by the service worker
   ...
   - **Role auto-resolution** (`role: 'auto'`): queries the `dps` metric, then `extractSpec()` reads the ranked spec from that same response (`allStars[].spec`, falling back to `rankings[].bestSpec`/`spec`) and `roleForSpec()` decides the role. A second `hps` query is issued **only** when `needsHpsFollowUp()` says so — a healer spec, or a response that reported nothing at all (see quirk 23). Returns the resolved `role` and `spec` on the score. Used by the scout pre-flight (no page to read a spec icon from) and by the two sites whose role markup is unreliable — WoWProgress rows and the WCL recruitment search. Sites with dependable role markup (Raider.IO, GoW) still send the role they read, which skips the spec round trip entirely
...
18. **Unit tests:** `tests/common.test.js` (Vitest) inlines copies of the content-script functions it covers (`normalizeClassName`, `failsWclThresholds`, `roleToMetric`, `characterKey`, `normalizeCharacter`, `extractCharacterFromUrl`) because content scripts aren't modules — keep the copies in sync when you touch the originals. `tests/scout.test.js` imports `src/scout.js` directly, and `tests/wcl-api.test.js` imports the pure response parsers (`extractScores`, `extractSpec`, `needsHpsFollowUp`) from `src/wcl-api.js` — that module only touches `chrome`/`fetch` inside functions, so it loads in Node. Run with `npm test`.
...
23. **`role: 'auto'` costs one request for most characters, two for healers.** WarcraftLogs bills a points budget by query complexity and `zoneRankings` is the expensive field, so asking for both metrics up front roughly doubled the cost of scoring a whole list — which matters on WoWProgress, where every row now uses `auto`. The dps response already carries the ranked spec, so the hps query is only issued for characters that need it. `needsHpsFollowUp()` also fires on a response with **no spec and no scores**: that is usually a character with no logs, but it is indistinguishable from a healer with no damage rankings, and scoring one of those as a DPS is the one wrong answer that matters. If WCL turns out to always rank healers on damage, that second condition can be dropped. When the follow-up also comes back empty the score keeps the neutral `dps` role rather than claiming `healer` on no evidence.
