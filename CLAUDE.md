...
4. If a tab did open, `warcraftlogs.js` still runs the older open-then-close check as a backstop — for lookups that failed transiently, or with pre-flight switched off. It scores through the same API, so **without credentials nothing closes at all** (the lookup answers `NO_CREDENTIALS`, which the backstop treats as transient). It also only closes a tab the background itself opened (quirk 54)
...
| `scoutPreflight` | boolean | `true` | Score a candidate via the API before opening their WCL tab; skip the tab entirely for rejects. Needs credentials — without them the tab opens and nothing closes it (the backstop needs the API too) |
...
53. **Pre-flight closes the source tab only if it still shows the candidate.** `scoutAndOpenTab` decides asynchronously — token exchange plus GraphQL, each up to the 10 s timeout — and WoWProgress character pages are usually opened *in the listing's own tab*. Closing `details.tabId` blindly on a reject therefore closed whatever that tab showed by the time the answer arrived: the gearscore listing after a Back, or the next candidate. `closeTabIfStillOn()` re-reads the tab (`url`, and `pendingUrl` while a navigation is in flight) and closes it only when `isSameWowProgressCharacter()` matches the page the decision was about — the property the pre-1.4 `closeWowProgressTab()` had by matching on URL. `tests/background-tabs.test.js` covers it.

54. **The WCL backstop closes only tabs the background opened.** `warcraftlogs.js` runs on *every* WarcraftLogs character page and reports `parseThresholdFailed` whenever the character is below threshold, and the background used to close the sender unconditionally — plus, by URL pattern, every WoWProgress tab showing that name. A WCL page the user opened themselves (bookmark, Discord link) was closed out from under them, contrary to the settings text ("closes an *auto-opened* WCL tab"). `scoutAndOpenTab` now records each tab it creates in `chrome.storage.session` under `scoutedTabs` (`{wclUrl, sourceTabId, sourceUrl, closeSource}`; session storage because the backstop can report after a Cloudflare wait longer than an idle worker lives), and `handleParseThresholdFailed()` acts only on a recorded tab still showing the character it was opened for. The source tab is closed through `closeTabIfStillOn()` (quirk 53), never by URL pattern. `tabs.onRemoved` drops the record so a reused id is never mistaken for ours.

55. **One scout per character per source tab per 30 seconds.** `webNavigation.onCompleted` fires for every *load*, not every visit: Cloudflare serves its challenge at the page's own URL and reloads it once passed, and a refresh or Back/Forward completes again too — each opened another identical WCL tab. `claimScout()` keys on source tab + `wclCharacterKey()` (name lower-cased; WCL names are case-insensitive), is claimed synchronously before any `await` so racing events still yield one tab, and lives in memory because the duplicates arrive while the worker is necessarily awake. A suppressed request answers `{ opened: false, verdict: 'duplicate' }`; Raider.IO's notice only fires on `reject`, so it stays quiet.

56. **Raider.IO keys its redirect on the character, not a boolean.** It is a single-page app, so the content script outlives a route change; a `redirected` flag opened a WCL tab for the first character of a session and never again. `warcraftLogsRedirectedFor` holds `characterPathKey()` (region/realm/name, lower-cased, so `/mythic-plus` sub-routes are the same visit). `tests/raiderio-redirect.test.js` drives the real script through `history.pushState`.

57. **An absent setting takes its documented default, in every reader.** Three readers used a truthy / `|| 0` check against a setting whose schema default is non-zero: `openWarcraftLogsTab` (auto-open silently off until the first Save), `hideRaiderIoAds` (ads shown), and the DPS parse thresholds (scored against 0/0 while the popup displayed 60/50). The mirror-image bug sat in both save paths: `parseInt(x) || 50` turned a typed `0` ("no minimum", which the inputs allow) back into the default, so a DPS threshold could never be switched off. `thresholdSetting()` (in both `common.js` and `preflight.js` — `tests/common.test.js` asserts the two agree) maps absent/blank/unparseable → default and keeps `0`; the schema's `allowZero` flag and the popup's `intOr()` do the same on save. When adding a default-`true` boolean, read it `!== false`.
